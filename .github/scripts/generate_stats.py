...
def langs_svg(data):
    ...
    if not lang_bytes:
        return '<svg xmlns="http://www.w3.org/2000/svg" width="300" height="100"><rect fill="' + T["card"] + '" width="300" height="100" rx="6"/><text x="150" y="55" text-anchor="middle" fill="' + T["text"] + '" font-size="14" font-family="sans-serif">No language data</text></svg>'
...
def streak_svg(data):
    ...
    return '\n'.join([
        ...
        '  ' + block(124, "Total Contributions", f"{total:,}", T["green"]) + block(248, "Current Streak", cur, T["orange"] if cur > 0 else T["text"]) + block(372, "Longest Streak", longest, T["purple"]),
        '</svg>',
    ])
